...
[34 39]
```
Note that in this case we have to use the **bitwise** `&` operator because the evaluation of `example_list>30` and `example_list<40` returns a vector of True and False values, so we need to compare two vectors instead of two values.
...
```
Note that in this case we have to use the **bitwise** `|` operator because the evaluation of `example_list<0` and `example_lis>10` returns a vector of True and False values, so we need to compare two vectors instead of two values.
