---
title: "Genome-Scale Phylogeny: From BUSCO Genes to Phylogenetic Tree"
author: "Misha Paauw"
date: 2025-10-31
engine: knitr
format:
...
categories: [comparative genomics, crunchomics, phylogeny]
description: "Learn how to plot create a core-genome phylogenetic tree in this blog post."
---
...
- Step 1: Downloading our genome panel of interest using `ncbi-datasets`
- Step 2: Identifying the presence of conserved genes using `busco`
- Step 3-5: Gathering all busco gene amino acid sequences in one place
- Step 6: Aligning the amino acid sequences using `MAFFT`
- Step 7: Building the phylogenetic tree using `iqtree2`
...
## Step 1: downloading microsporidium genomes

First, head to the [NCBI Genome](https://www.ncbi.nlm.nih.gov/datasets/genome/) website and search for your taxon of interest. In our case, we retrieved a table containing the metadata of all available microsporidia genome assemblies (n = 124). Then, we pasted the `GCA_xx` accession numbers in a simple text file (here: `msp_genomes_genbank.txt`). We will then use the large genome downloads [instruction by NCBI](https://www.ncbi.nlm.nih.gov/datasets/docs/v2/how-tos/genomes/large-download/) to download the fasta files containing the genome assemblies.

```{bash filename="bash"}
#| eval: false
(base) [mpaauw@omics-h0 msp_phylogeny]$ head -n 4 msp_genomes_genbank.txt # <1>
GCA_036630325.1
GCA_000091225.2
GCA_000146465.1
GCA_022605425.2
...
datasets download genome accession --inputfile msp_genomes_genbank.txt --dehydrated --filename /scratch/misha/msp_phylogeny/msp_genomes.zip # <2>
unzip /scratch/misha/msp_phylogeny/msp_genomes.zip -d /scratch/misha/msp_phylogeny/msp_genomes
datasets rehydrate --directory /scratch/misha/msp_phylogeny/msp_genomes # <3>
```

1. Inspect the file containing accession numbers. Looks good, one accession at each line.
2. This command downloads a 'dehydrated' data package first. This is not usable yet, but don't worry, we will 'rehydrate' this into fasta files soon!
3. Here we 'rehydrate' the data package, that is, it actually downloads the fasta files of the genome assemblies. I was impressed with the speed of downloading 124 genomes!

## Step 2: identifying BUSCO genes
...
for fna in /scratch/misha/msp_phylogeny/msp_genomes/ncbi_dataset/data/*/*.fna; do # <1>
    genome=$(basename "$(dirname "$fna")") # <2>
    outpath="$OUTDIR/${genome}"
    busco -i $fna -l microsporidia_odb12 -m genome -o $genome -c 12 --out_path $OUTDIR # <3>
done
```

1. Here, we use two wildcards (`*`) to define a list of all the `.fna` files that were downloaded. 
2. Here, we strip the long pathname down to just the genome identifier, e.g. `GCA_1234`
3. This is the actual busco command. We use lineage `mirosporidia_odb12`, we use genome mode (`-m genome`), and specify the use of 12 threads (`-c 12`).
...
## Step 3: finding the most conserved BUSCOs
...
for file in $(find /scratch/misha/msp_phylogeny -type f -name "full_table.tsv"); do # <1>
	echo $file
	grep -v "^#" ${file} | awk '$2=="Complete" {print $1}' >> all_detected_buscos.txt; # <2>
done

cat all_detected_buscos.txt | sort | uniq -c | sort | tail -n 250 | column -t | cut -d ' ' -f 3 > conserved_buscos.txt # <3>
```

1. This line loops through all the files that satisfy the `find` condition, that is: the output files `full_table.tsv` from each BUSCO run.
2. Here, we parse each `full_table.tsv` file to get rid of comment fields (`grep -v "^#"`), and extract each 'complete' BUSCO ID that was identified. Each BUSCO ID that was detected in different genomes will occur multiple times in this list. Yes, that's a long list!
3. Here, we `sort` the long list and count (`uniq -c`) how often each BUSCO ID was detected. We then `sort` again to see which BUSCOs are found most often. Taking the `tail -n 250` gives us the IDs of the 250 most conserved BUSCO genes in our genome set.

## Step 4 & 5: getting the BUSCO sequences from each genome
...
for genome in $(find /scratch/misha/msp_phylogeny -type d -name "single_copy_busco_sequences"); do # <1>
	accession=$(echo "$genome" | awk -F'/' '{print $6}') # <2>
	while read busco; do # <3>
		cp $genome/$busco.faa busco_aa/$busco-$accession.faa # <4>
		sed -i "s/^>.*/>${accession}/g" busco_aa/$busco-$accession.faa # <5>
	done < conserved_buscos.txt
done
```

1. This is the outer loop, over the BUSCO output of each genome.
2. Here, we extract the accession ID from the `genome` pathname. In my case the accession ID was in column 6, this may vary!
3. This is the inner loop, over each BUSCO ID.
4. Here we copy the amino acid sequence from the busco results, to the local folder. We rename the file to fit `buscoID-accession.faa` format.
5. We use `sed` to add accession information to fasta header.
...
```{bash filename="bash"}
#| eval: false
(busco_2025) [mpaauw@omics-h0 msp_phylogeny]$ cat 5_concatenate_seqs.sh 
while read busco; do
	cat busco_aa/$busco-*.faa >> busco_aa_concat/$busco-all.faa;
done<conserved_buscos.txt
```

Let's do a sanity check: 

1. We choose to analyse 250 BUSCO genes, so we should have 250 concatenated amino acid fasta files. 
2. As each BUSCO is present in the majority of microsporidium genomes, I expect about 250 * 115 ~= 28.000 sequence entries among those 250 multiline fasta files.

```{bash filename="bash"}
#| eval: false
# Sanity check 1: correct
(base) [mpaauw@omics-h0 msp_phylogeny]$ ls busco_aa_concat/ | wc -l
250

# Sanity check 2: correct
(base) [mpaauw@omics-h0 msp_phylogeny]$ cat busco_aa_concat/*.faa | grep -c ">"
28814
```

# Step 6: making multiple sequence alignments
...
```{bash filename="bash"}
#| eval: false
(mafft) [mpaauw@omics-h0 msp_phylogeny]$ cat 6_align_buscos.sh 
while read busco; do
	echo "aligning ${busco}"
	mafft --thread 32 --quiet --localpair --maxiterate 250 busco_aa_concat/$busco-all.faa > busco_aligned/$busco-aligned.faa;
done<conserved_buscos.txt
```

# Step 7: inferring the phylogenetic tree
...
iqtree2 -s concatenated_alignment -p concatenated_alignment.nex -m Q.yeast+F+I+R6 -T 32 -B 1000 --prefix output_tree # <1>

(iqtree) [mpaauw@omics-h0 msp_phylogeny]$ sbatch 7_tree_slurm.sh # submit the job!
```

1. `iqtree` needs the concatenated alignment, a partition file, and the model you selected (if you don't want to run ModelFinder again). Furthermore, we specify that `iqtree` can use 32 threads (`-T 32`), and that we want 1000 ultrafast bootstrap replicates (`-B 1000`).
...
Total wall-clock time used: 27002.981 sec (7h:30m:2s)
```

Wow, the total CPU time to make this tree was 218 hours! That's 9 days! Luckily, thanks to using 32 threads in parallel, it only took a wall-clock time of 7 hours and 30 minutes. Now, let's finally look at the phylogenetic tree. The final tree is stored `.contree` output file and can be visualized in iTOL (for example):

![](post_images/msp_tree.png)

Isn't that wonderful! Some things to consider here at this point are:

1. Rooting the tree with an outgroup. Here, we used the genome of *Metchnikovella*, known to be an early-branching species in Microsporidium.
2. Comparing the tree topology to published phylogenies. Major disagreements might reveal an interesting finding, but could also point to potential issues with your new tree. In our case, the major clade groupings seem to recapitulate the microsporidium literature (e.g. [Khalaf et al, 2025](https://www.biorxiv.org/content/10.1101/2025.05.12.652816v1.abstract)) pretty well.
3. Check bootstrap values: are the splits in your tree well supported, or should a level of uncertainty be acknowledged?
4. Which metadata would be interesting to visualize alongside the tree? In our case, I would be interested in displaying the hosts species that are infected by these species. Do fish-infecting isolates cluster together in the phylogenetic tree, or has this phenotype evolved independently multiple times?
...
- Parts of this workflow was inspired by [this blogpost by Rick Masonbrick](https://bioinformaticsworkbook.org/phylogenetics/reconstructing-species-phylogenetic-tree-with-busco-genes-using-maximum-liklihood-method.html#gsc.tab=0).
